...
\begin{itemize}
    ...
    \end{itemize}
    This is done through the \textit{/users/validate-token} endpoint of the User Manager.
    ...
    \end{itemize}
    \item \textbf{Game History -> User-Manager:} Game History needs to connect to User Manager also to get a list of
    (updated) usernames from a list of user-ids to return the leaderboard to users.\\
    This is done through the \texttt{/users/usernames-by-ids} endpoint of the User Manager.
    \item \textbf{User Editor:}
    We've separated the user profile editing and data viewing endpoints from the user manager to keep the user manager smaller
    ...
    \begin{itemize}
        \item \texttt{/internal/update-password}
        \item \texttt{/internal/update-email}
        \item \texttt{/internal/update-username}
        \item \texttt{/users/my-username-my-email}
    \end{itemize}
    ...
    The Game Engine will connect to this microservice to get the deck selected from the user: the user provides the deck's number
    and the Game Engine asks for the corresponding cards to the Collection. This is done through the endpoint \\
    \texttt{/collection/user-decks?user=\{user\_id\}\&slot=\{slot\_number\}}.
    \item \textbf{Game History and Leaderboard:} The Game Engine delegates the Game History to store finished games. How this is handled is discussed in detail in the \hyperref[sec:rabbitmq]{Additional Features} section.\\
