...
\textbf{Result:} One low-severity vulnerability found in pip version (fixed updating it).
...
\begin{itemize}[noitemsep]
    \item \textbf{RabbitMQ:} Upgraded to \texttt{rabbitmq:4.2.1-alpine} to eliminate critical vulnerabilities
    \item \textbf{MongoDB:} Switched to \texttt{mongo:8.0.4-noble} for minimal vulnerability exposure
\end{itemize}
