...
\subsection{Deck Building Constraints}
...
\begin{itemize}
    ...
    \item \textbf{Cost Limit:} For any suit, the total point value of the two cards \textbf{must not exceed 15}.
\end{itemize}
...
\subsection{Combat Hierarchy}
\label{sec:combat-hierarchy}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Draw Phase:} At the end of every turn, each player draws \textbf{1 card} to get back to \textbf{3}.
        \item \textbf{Battle Phase:} Both players play one card \textbf{face down}, then reveal it at the same time.
        \item \textbf{Scoring:} Determine the winner based on the \hyperref[sec:combat-hierarchy]{Combat Hierarchy}. The winner earns \textbf{1 point}. If it's a tie, both players earn \textbf{1 point}.
    \end{itemize}
    \item \textbf{Victory:} The game ends immediately when one of the players reaches \textbf{5 points}.
\end{enumerate}

\subsection{Example of a Match}
